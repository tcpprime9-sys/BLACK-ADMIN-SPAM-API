...
def StarT_SerVer():
    print(render("BLACK ADMIN X ", colors=["white", "yellow"], align="center"))
    TexT = f"[TarGeT InFo] > BoTs arE OnLine\n[BoT sTaTus] > [bold green]ConEcTed SuccEssFuLy[/bold green]"
    panel = Panel(
        Align.center(TexT),
        title="[bold yellow]FF - BLACK ADMIN X [/bold yellow]",
    ...
    forced_target_input = input("Enter target UID to force (IF YOU ARE BAD _-_-_BLACK ADMIN X  IS YOUR DAD🥶): ").strip()
